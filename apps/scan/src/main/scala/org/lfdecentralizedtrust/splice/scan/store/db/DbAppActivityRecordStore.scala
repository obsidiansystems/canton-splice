...
object DbAppActivityRecordStore {
  ...
  def apply(
    ...
    // Use store_name to make history_id per-synchronizer (same pattern as traffic summaries)
    val storeName = s"app-activity-${synchronizerId.toProtoPrimitive}"
    getOrCreateHistoryId(storage, party, participantId, storeName).map { historyId =>
      new DbAppActivityRecordStore(storage, historyId, synchronizerId, loggerFactory)
    }
  }
